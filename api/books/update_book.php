...
if (stripos($contentType, 'application/json') === 0) {
    ...
    if (isset($_POST['copy_ids']) && is_string($_POST['copy_ids'])) {
        ...
        $payload->copy_ids = $decoded ?? $_POST['copy_ids'];
    }
    if (isset($_POST['copy_locations']) && is_string($_POST['copy_locations'])) {
        $decoded = json_decode($_POST['copy_locations'], true);
        $payload->copy_locations = $decoded ?? $_POST['copy_locations'];
    }
    if (isset($_POST['course_ids']) && is_string($_POST['course_ids'])) {
        $decoded = json_decode($_POST['course_ids'], true);
        $payload->course_ids = $decoded ?? $_POST['course_ids'];
    }
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $filePath)) {
            $coverFile = 'uploads/books/' . $fileName;
            $payload->pic_path = $coverFile;
        }
    }
    ...
    if (isset($_FILES['pdf_file']) && $_FILES['pdf_file']['error'] === UPLOAD_ERR_OK) {
        ...
        $pdfFileName = uniqid('pdf_') . '_' . time() . '.' . pathinfo($_FILES['pdf_file']['name'], PATHINFO_EXTENSION);
        ...
        if (move_uploaded_file($_FILES['pdf_file']['tmp_name'], $pdfFilePath)) {
            ...
            $payload->pdf_url = $pdfFile;
        }
    }
    
    error_log('UPDATE_BOOK FORM DATA: ' . json_encode($payload));
}
...
// Check both the payload pdf_url and the uploaded file path
$pdfUrl = null;
...
try {
    ...
    if (!empty($pdfUrl)) {
        ...
        // Get existing PDF path to compare
        $existingStmt = $db->prepare('SELECT file_path FROM Digital_Resources WHERE isbn = :isbn AND resource_type = "PDF"');
        $existingStmt->execute([':isbn' => $payload->isbn]);
        $existingPdf = $existingStmt->fetch(PDO::FETCH_ASSOC);
        
        $existingPath = $existingPdf ? $existingPdf['file_path'] : null;
        
        // Only delete the old file and re-insert if the path has actually changed
        // This prevents accidentally deleting the file when the same path is sent back from the edit form
        if ($existingPath !== $pdfUrl) {
            // Delete old PDF file from filesystem if it exists
            if ($existingPath && !empty($existingPath)) {
                $oldFilePath = '../../' . $existingPath;
                if (file_exists($oldFilePath)) {
                    @unlink($oldFilePath);
                }
            }
            
            // Remove existing PDF record for isbn
            ...
            error_log('UPDATE_BOOK: Updated PDF for ISBN ' . $payload->isbn . ' from "' . ($existingPath ?? 'null') . '" to "' . $pdfUrl . '"');
        } else {
            error_log('UPDATE_BOOK: PDF path unchanged for ISBN ' . $payload->isbn . ', skipping update');
        }
    }
