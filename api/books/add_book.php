...
try {
    ...
    if (!empty($pdfUrl)) {
        // Extract file name from URL or use the URL as filename
        $fileName = basename(parse_url($pdfUrl, PHP_URL_PATH));
        if (empty($fileName)) {
            $fileName = 'document.pdf';
        }
        
        $pdfStmt = $db->prepare('INSERT INTO Digital_Resources (isbn, file_name, resource_type, file_path)
            VALUES (:isbn, :file_name, "PDF", :file_path)
